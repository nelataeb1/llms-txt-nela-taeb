...
import { z } from "zod";
import { evaluateRetrieval } from "@/lib/eval";
...
/** Scores how well an agent can find the right page from the generated index. */
export async function POST(request: Request) {
  ...
  try {
    return NextResponse.json({ report: await evaluateRetrieval(job.result.document) });
  } catch (error) {
